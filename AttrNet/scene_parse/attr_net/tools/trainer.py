...
class Trainer:
    ...
    def train(self):
        ...
        att_len_list = [0, 7, 4, 12, 23, 28, 26, 2]  #@ if use "선수상황"(len 2), "선수방향"(len 4) -> [0, 2, 4], make_basketball_obj_json.py att_bind에서 앞에 위치한 att의 len 먼저 적어야 함
        ...
        while t < self.num_iters:
            ...
            if epoch % self.val_epochs == 0 :
                if self.val_loader is not None:    
                    ...
                    self.model.save_checkpoint(f'{self.run_dir}/checkpoint_{epoch}.pt', self.opt)
    ...
    def check_val_loss(self, att_len_list):
        ...
        score = torch.zeros((1, len(att_len_list[1:]))).cuda()
        total = 0
        for x, y in self.val_loader:
            ...
            score = torch.add(score, self.model.check_correct(att_len_list).cuda())
        ...
        #*####
        score = torch.flatten(score).detach()
        acc = [str(float(score[i] / total))[:8] for i in range(score.shape[0])]
