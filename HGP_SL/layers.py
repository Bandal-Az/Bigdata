...
class HGPModel(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, pool_ratio=0.8):
        super(HGPModel, self).__init__()
        # 1단 GCN
        self.gcn1 = GCN(in_channels, hidden_channels)
        # Graph Pooling
        self.pool = HGPSLPool(hidden_channels, ratio=pool_ratio)
        # 2단 GCN
        self.gcn2 = GCN(hidden_channels, out_channels)

    def forward(self, x, edge_index, edge_attr=None):
        # 1단 GCN
        x = self.gcn1(x, edge_index)
        # Pooling
        x, edge_index, edge_attr, batch = self.pool(x, edge_index, edge_attr)
        # 2단 GCN
